...
class EmailNote implements ServiceInterface {
	...
	public function render( WC_Order $order, bool $sent_to_admin, bool $plain_text, mixed $email ): void { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
		...
		if ( $plain_text ) {
			echo "\n" . \esc_html( \sprintf( /* translators: %s: invoice number */ __( 'e-Financials invoice: %s', 'arbictus-sync-for-e-arveldaja' ), $label ) ) . "\n";

			return;
		}

		echo '<p>' . \esc_html( \sprintf( /* translators: %s: invoice number */ __( 'e-Financials invoice: %s', 'arbictus-sync-for-e-arveldaja' ), $label ) ) . '</p>';
	}
}
