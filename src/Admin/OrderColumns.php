...
class OrderColumns implements ServiceInterface {
	...
	public function add_column( array $columns ): array {
		...
		$label = __( 'e-Financials', 'arbictus-sync-for-e-arveldaja' );
	...
	private function cell_html( WC_Order $order ): string {
		...
		if ( $credit > 0 ) {
			$parts[] = \esc_html__( 'credit', 'arbictus-sync-for-e-arveldaja' );
		}

		if ( $error !== '' ) {
			$parts[] = '<span style="color:#b32d2e" title="' . \esc_attr( $error ) . '">' . \esc_html__( 'error', 'arbictus-sync-for-e-arveldaja' ) . '</span>';
		}
