...
class OrderActions implements ServiceInterface {
	...
	public function register_actions( array $actions ): array {
		...
		$actions[ self::ACTION_SYNC ]    = __( 'Send / resend to e-Financials', 'arbictus-sync-for-e-arveldaja' );
		$actions[ self::ACTION_DELIVER ] = __( 'Deliver e-Financials invoice', 'arbictus-sync-for-e-arveldaja' );
	...
	public function handle_deliver( WC_Order $order ): void {
		...
		if ( $invoice_id <= 0 ) {
			$order->add_order_note( __( 'Cannot deliver: no e-Financials invoice on this order.', 'arbictus-sync-for-e-arveldaja' ) );
			$order->save();
		...
		try {
			...
			$order->add_order_note(
				\sprintf(
					/* translators: %s: error */
					__( 'e-Financials deliver failed: %s', 'arbictus-sync-for-e-arveldaja' ),
					$message
				)
			);
			$order->save();
