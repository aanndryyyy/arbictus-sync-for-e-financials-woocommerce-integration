...
final class RemoteSelectOptions {
	...
	public static function with_saved( array $options, string $saved ): array {
		...
		$entry = [
			...
			$saved => \sprintf( __( 'ID %s (currently saved)', 'arbictus-sync-for-e-arveldaja' ), $saved ),
		];
	...
	private static function grouped( array $cash_or_bank, array $other ): array {
		...
		if ( $cash_or_bank !== [] ) {
			$groups[ __( 'Cash and bank accounts', 'arbictus-sync-for-e-arveldaja' ) ] = $cash_or_bank;
		}

		if ( $other !== [] ) {
			$groups[ __( 'Other accounts', 'arbictus-sync-for-e-arveldaja' ) ] = $other;
		}
