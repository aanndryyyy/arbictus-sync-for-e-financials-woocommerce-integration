...
class EFinancialsIntegration extends \WC_Integration {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'e-Financials', 'arbictus-sync-for-e-financials-and-woocommerce' );
		$this->method_description = __( 'Sync WooCommerce orders to e-Arveldaja / e-Financials in the background.', 'arbictus-sync-for-e-financials-and-woocommerce' );
	...
	public function init_form_fields(): void {
		...
		$this->form_fields = [
			'api_section'                              => [
				'title' => __( 'API connection', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'  => 'title',
			],
			self::SETTING_KEY_API_KEY_ID               => [
				'title'       => __( 'API Key ID', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'text',
				'description' => __( 'View guide <a href="https://abiinfo.rik.ee/en/node/303">here</a>.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'desc_tip'    => false,
				'default'     => '',
			],
			self::SETTING_KEY_API_KEY_PUBLIC           => [
				'title'    => __( 'API Key Public', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'     => 'text',
				'desc_tip' => false,
				'default'  => '',
			],
			self::SETTING_KEY_API_KEY_PASSWORD         => [
				'title'    => __( 'API Key Password', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'     => 'password',
				'desc_tip' => false,
				'default'  => '',
			],
			self::SETTING_KEY_API_ENVIRONMENT          => [
				'title'       => __( 'API Environment', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'select',
				'label'       => __( 'Choose the environment', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default'     => self::SETTING_KEY_API_ENVIRONMENT_OPTION_TEST,
				'description' => __( 'View <a href="https://demo-rmp.rik.ee">test environment</a> or <a href="https://e-arveldaja.rik.ee/">live environment</a>.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'options'     => [
					self::SETTING_KEY_API_ENVIRONMENT_OPTION_TEST => __( 'Test Environment', 'arbictus-sync-for-e-financials-and-woocommerce' ),
					self::SETTING_KEY_API_ENVIRONMENT_OPTION_LIVE => __( 'Live Environment', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				],
			],
			'invoice_section'                          => [
				'title' => __( 'Invoicing', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'  => 'title',
			],
			self::SETTING_KEY_INVOICE_SERIES_ID        => [
				'title'       => __( 'Invoice series', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'select',
				'description' => __( 'Number prefix of the selected series is sent as number_prefix on every sale invoice. Leave empty to let e-Financials number invoices itself.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default'     => '',
				'class'       => 'wc-enhanced-select',
				'options'     => $series_options,
			],
			self::SETTING_KEY_TEMPLATE_ID              => [
				'title'       => __( 'Invoice template', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'select',
				'description' => __( 'Sale invoice template (cl_templates_id). Required before first sync.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default'     => '',
				'class'       => 'wc-enhanced-select',
				'options'     => $template_options,
			],
			self::SETTING_KEY_SALE_ARTICLE_ID          => [
				'title'       => __( 'Default sale article', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'select',
				'description' => __( 'Required: e-Financials refuses to create products without a sale account, and books VAT by article. Its VAT rate must match the rate your shop charges.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default'     => '',
				'class'       => 'wc-enhanced-select',
				'options'     => $article_options,
			],
			self::SETTING_KEY_SALE_ARTICLE_MAP         => [
				'title'       => __( 'VAT rate → sale article map (JSON)', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'textarea',
				'description' => __( 'Required for mixed-rate catalogues and for 0% lines — including shops with WooCommerce taxes switched off, where every line is 0%. Example: {"22":1,"9":5,"0":12}. Each order line uses the article mapped to its WooCommerce tax rate; unmapped rates fall back to the default article and sync fails if the rates disagree.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default'     => '',
				'css'         => 'width:100%;min-height:80px;font-family:monospace',
			],
			self::SETTING_KEY_TERM_DAYS                => [
				'title'   => __( 'Payment term (days)', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'    => 'number',
				'default' => '14',
			],
			self::SETTING_KEY_USE_WC_ORDER_NUMBER      => [
				'title'   => __( 'Use WooCommerce order number as invoice suffix', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Push WC order number as number_suffix', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default' => 'yes',
			],
			'payment_section'                          => [
				'title'       => __( 'Payment recording', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'title',
				'description' => __( 'Gateway-agnostic: maps WooCommerce payment method ids to cash fields or transactions. No gateway plugin required.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
			],
			self::SETTING_KEY_DEFAULT_PAYMENT_MODE     => [
				'title'   => __( 'Default payment mode', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'    => 'select',
				'default' => SettingsRepository::PAYMENT_MODE_CASH,
				'options' => [
					SettingsRepository::PAYMENT_MODE_CASH => __( 'Cash fields on invoice', 'arbictus-sync-for-e-financials-and-woocommerce' ),
					SettingsRepository::PAYMENT_MODE_TRANSACTION => __( 'Payment transaction', 'arbictus-sync-for-e-financials-and-woocommerce' ),
					SettingsRepository::PAYMENT_MODE_OFF  => __( 'Off (invoice only)', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				],
			],
			self::SETTING_KEY_DEFAULT_CASH_ACCOUNTS_ID => [
				'title'       => __( 'Default cash account', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'select',
				'description' => __( 'Used for Option A (paid_in_cash) when the gateway map does not override.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default'     => '',
				'class'       => 'wc-enhanced-select',
				'options'     => $cash_account_options,
			],
			self::SETTING_KEY_DEFAULT_ACCOUNTS_DIMENSIONS_ID => [
				'title'       => __( 'Default accounts dimension', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'select',
				'description' => __( 'Used for Option B (transactions) when the gateway map does not override.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default'     => '',
				'class'       => 'wc-enhanced-select',
				'options'     => $dimension_options,
			],
			self::SETTING_KEY_GATEWAY_MAP              => [
				'title'       => __( 'Per-gateway payment map (JSON)', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'        => 'textarea',
				'description' => __( 'Example: {"bacs":{"mode":"transaction","accounts_dimensions_id":4},"cod":{"mode":"cash","cash_accounts_id":1010}}. Empty uses built-in defaults for bacs/cheque/cod.', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default'     => '',
				'css'         => 'width:100%;min-height:120px;font-family:monospace',
			],
			'delivery_section'                         => [
				'title' => __( 'Delivery & products', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'  => 'title',
			],
			self::SETTING_KEY_AUTO_DELIVER             => [
				'title'   => __( 'Auto-deliver invoice email after register', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Send PDF email via e-Financials deliver API', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default' => 'no',
			],
			self::SETTING_KEY_AUTO_DELIVER_EINVOICE    => [
				'title'   => __( 'Also send e-invoice (XML) when available', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'send_einvoice=true when can_send_einvoice', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default' => 'no',
			],
			self::SETTING_KEY_PRODUCT_AUTO_SYNC        => [
				'title'   => __( 'Auto-sync products on save', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Upsert e-Financials products when WooCommerce products are saved', 'arbictus-sync-for-e-financials-and-woocommerce' ),
				'default' => 'no',
			],
		];
	}
	...
	private function maybe_connection_ping(): void {
		...
		try {
			...
			\WC_Admin_Settings::add_message( __( 'e-Financials connection OK.', 'arbictus-sync-for-e-financials-and-woocommerce' ) );
		} catch ( Throwable $e ) {
			\WC_Admin_Settings::add_error(
				\sprintf(
					/* translators: %s: error */
					__( 'e-Financials connection failed: %s', 'arbictus-sync-for-e-financials-and-woocommerce' ),
	...
	private function safe_id_options( string $bucket, string $setting_key, callable $callback ): array {
		...
		$blank = [ '' => __( '— Select —', 'arbictus-sync-for-e-financials-and-woocommerce' ) ];
		...
		try {
			...
			return RemoteSelectOptions::with_saved(
				[ '' => __( 'Could not load options — check credentials and the WooCommerce logs', 'arbictus-sync-for-e-financials-and-woocommerce' ) ],
				$saved
			);
		}
