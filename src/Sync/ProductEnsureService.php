...
class ProductEnsureService {
			...
			throw new RuntimeException(
				\esc_html(
					\sprintf(
						...
						__( 'No e-Financials sale article is configured for VAT %s%%. Set a default sale article or add the rate to the VAT rate map.', 'arbictus-sync-for-e-arveldaja' ),
						(string) $vat_rate
					)
				)
			);
			...
			throw new RuntimeException(
				\esc_html(
					\sprintf(
						...
						__( 'Sale article #%1$d is VAT %2$s%% but the order line is VAT %3$s%%. Add "%3$s" to the VAT rate → sale article map; e-Financials books VAT by article, so the rates must match. Shops with WooCommerce taxes disabled need a "0" entry.', 'arbictus-sync-for-e-arveldaja' ),
						$sale_article,
						(string) $article_rate,
						(string) $vat_rate
					)
	...
	private function find_or_create( string $code, string $name, array $params ): int {
		...
		if ( $sale_article <= 0 ) {
			throw new RuntimeException(
				\esc_html__( 'e-Financials requires a default sale article before products can be created. Set it in the integration settings.', 'arbictus-sync-for-e-arveldaja' )
			);
		}
